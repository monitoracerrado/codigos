...
#include <MQ135.h>                ////https://github.com/zesteros/ArduinoAirQualityMonitor/       Air sensor library
//Library allows either I2C or SPI, so include both.
#include <Wire.h>                 //https://github.com/esp8266/Arduino/blob/master/libraries/Wire
#include <SPI.h>                  //https://github.com/PaulStoffregen/SPI/blob/master

// Variaveis Servidor MQTT
char mqtt_station[30];
...
// Parametros do Sensor BME280
// BME280 bme(D1,D2);


// Servidor de Localizacao
#define GOOGLE_KEY "AIzaSyBfDb1hQivxHZoGg-iHHN-ep6xOT1QY6ww"      // Chave da API do Google Maps
...
void datalocal() {
    ...
    Serial.println("Sem Georeferenciando.");
}

void dataclima() {
 // Dados BME280
  float hb = bme.readFloatHumidity();
  float tb = bme.readTempC();
  float pb = (bme.readFloatPressure() / 100.0F);
  float ab = bme.readFloatAltitudeMeters();
  ...
  Serial.print(bme.readFloatPressure() / 100.0F, 2);
  Serial.println(" hPa");
...
// Leitura dos dados do sensor MQ135
  float MQ135A = analogRead(A0);//air sensor voltage
  int MQ135D = digitalRead(2);//return 1 if ppm is low, and 0 when is a lot
  float co2 = mqSensor.getCO2PPM();//CO2 ppm
  float co = mqSensor.getCOPPM();//CO ppm
  float ethanol = mqSensor.getEthanolPPM();//Ethanol ppm
  float nh4 = mqSensor.getNH4PPM(); //NH4 ppm
  float toluene = mqSensor.getToluenePPM(); //Toluene ppm
  float acetone = mqSensor.getAcetonePPM(); //Acetone ppm
